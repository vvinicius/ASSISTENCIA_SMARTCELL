...
class ComprasController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        // Busca todos os clientes (id, nome e cpf)
        $clientes = Cliente::find()->select(['id', 'nome', 'cpf'])->all();

        // Cria um array com 'id' como chave e 'nome (cpf)' como valor
        $lista = [];
        foreach ($clientes as $cliente) {
            $lista[$cliente->id] = $cliente->nome . ' CPF(' . $cliente->cpf . ')';
        }

        // Verifica se a requisição é POST e tenta salvar o modelo
        if ($this->request->isPost) {
            if ($model->load($this->request->post()) && $model->save()) {
                return $this->redirect(['view', 'id' => $model->id]);
            }
        } else {
            // Carrega os valores padrão para novos registros
            $model->loadDefaultValues();
        }

        // Renderiza a view 'create', passando o model e a lista de clientes
